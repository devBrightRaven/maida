...
> **Windows で初めてインストール?** SmartScreen が警告を表示します。Maida はコード署名されていないためです(証明書は年 300 ドル以上)。**詳細情報** → **実行** をクリック。ソースコードは公開([MIT](LICENSE))、既定でテレメトリ送信なし。詳細は[マニュアル](https://maida.brightraven.world/manual/)の Before You Install セクションをご覧ください。
>
> **ROG Ally をご利用?** Armoury Crate をゲームパッドモードに設定するか、Maida をゲームプロファイルに追加してください。ボタンがゲームパッド信号として送信されます。
